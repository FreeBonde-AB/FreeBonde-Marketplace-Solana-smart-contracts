...
const TopBar = () => {
    ...
    useEffect(() => {
        if (publicKey) {
            ...
            setNickname(saved || `用户_${publicKey.toString().slice(0, 6)}`);
        }
    }, [publicKey]);
    ...
    const avatarUrl = publicKey ? USER_IMAGE_URLS[getAvatarIndex(publicKey.toString())] : USER_IMAGE_URLS[0];
            ...
            <div className="flex items-center space-x-3 px-4 cursor-pointer">
                ...
                {connected && (
                    <div className="flex items-center space-x-2">
                        <img src={avatarUrl} alt="avatar" className="w-10 h-10 rounded-full border-2 border-white" />
                        {editing ? (
                            <div className="flex items-center">
                                <input
                                    ...
                                    autoFocus
                                />
                                <button className="ml-1 text-xs text-yellow-300" onClick={handleNicknameSave}>Save</button>
                            </div>
                        ) : (
                            <span
